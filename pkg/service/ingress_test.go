...
func TestCreateURLPullIngressScheme(t *testing.T) {
	...
	t.Run("other schemes unaffected by the udp option", func(t *testing.T) {
		...
		// rtsp and rtsps are accepted schemes in this fork, so the rejection is
		// asserted with a scheme neither upstream nor this fork accepts.
		for _, url := range []string{"rtsp://1.2.3.4/live", "rtsps://1.2.3.4/live"} {
			svc, _ := newService(false)
		...
		_, err := svc.CreateIngress(adminCtx(), createReq("ftp://1.2.3.4/live"))
		require.Error(t, err)
		require.Contains(t, err.Error(), "invalid url scheme ftp")
		// END OPENVIDU BLOCK
	})
}
